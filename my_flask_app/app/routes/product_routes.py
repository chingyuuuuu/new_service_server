...
def save_product():
    ...
    if 'image' in request.files:
        ...
        image_url = f"http://140.136.151.74:5000/{image_path.replace(os.sep, '/')}"
...
def get_products():
    ...
    try:
            ...
            for product in products:
                if product.image:
                    image_url = f"http://140.136.151.74:5000/{product.image}" if '/uploads/' not in product.image else product.image
                else:
                    image_url = None
